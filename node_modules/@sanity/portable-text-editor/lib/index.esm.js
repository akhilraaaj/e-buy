...
import React, { createContext, useContext, useMemo, useEffect, useRef, useState, startTransition, useCallback, createRef, useLayoutEffect, forwardRef } from 'react';
import { defer, tap, switchMap, of, EMPTY, concatMap, Subject } from 'rxjs';
import { Schema } from '@sanity/schema';
import debug$m from 'debug';
import { randomKey } from '@sanity/util/content';
import { ReactEditor, withReact, Slate, useSlate, useSlateStatic, useSelected, Editable } from '@sanity/slate-react';
...
function debugWithName(name) {
  ...
  if (debug$m && debug$m.enabled(namespace)) {
    return debug$m(namespace);
  }
  return debug$m(rootName);
}
...
function isEqualToEmptyEditor(children, types) {
  var _a;
  return children === void 0 || children && Array.isArray(children) && children.length === 0 || children && Array.isArray(children) && children.length === 1 && Element$1.isElement(children[0]) && children[0]._type === types.block.name && "style" in children[0] && children[0].style === types.styles[0].value && !("listItem" in children[0]) && Array.isArray(children[0].children) && children[0].children.length === 1 && Text.isText(children[0].children[0]) && children[0].children[0]._type === "span" && !((_a = children[0].children[0].marks) == null ? void 0 : _a.join("")) && children[0].children[0].text === "";
}
...
const debug$l = debugWithName("operationToPatches");
function createOperationToPatches(types) {
  const textBlockName = types.block.name;
  function insertTextPatch(editor, operation, beforeValue) {
    debug$l("Operation", JSON.stringify(operation, null, 2));
  ...
  function removeNodePatch(editor, operation, beforeValue) {
    ...
    if (operation.path.length === 1) {
      ...
      debug$l("Span not found in editor trying to remove node");
      return [];
    } else {
      throw new Error("Unexpected path encountered: ".concat(JSON.stringify(operation.path)));
    }
...
const debug$k = debugWithName("API:editable");
  ...
  return function withEditableAPI(editor) {
    portableTextEditor.setEditable({
        ...
        if (selection) {
          ...
          if (range) {
            if (!(options == null ? void 0 : options.mode) || (options == null ? void 0 : options.mode) === "selected") {
              debug$k("Deleting content in selection");
            ...
            const nodes = Editor.nodes(editor, {
              ...
              match: node => {
                if ((options == null ? void 0 : options.mode) === "blocks") {
                  debug$k("Deleting blocks touched by selection");
                  return editor.isTextBlock(node) || !editor.isTextBlock(node) && Element$1.isElement(node);
                }
                debug$k("Deleting children touched by selection");
      ...
      removeAnnotation: type => {
        ...
        debug$k("Removing annotation", type);
        if (selection) {
          if (Range.isCollapsed(selection)) {
...
function createPatchToOperations(schemaTypes) {
  ...
  function insertPatch(editor, patch) {
    if (patch.path.length === 1) {
      ...
      debug$j("Inserting blocks at path [".concat(normalizedIdx2, "]"));
      debugState(editor, "before");
    ...
    debug$j("Inserting children at path ".concat(targetPath));
    debugState(editor, "before");
  ...
  function setPatch(editor, patch) {
    ...
    if (isTextBlock) {
      debug$j("Setting nodes at ".concat(JSON.stringify(patch.path), " - ").concat(JSON.stringify(targetPath)));
      debug$j("Value to set", JSON.stringify(value, null, 2));
      debugState(editor, "before");
      if (targetPath.length === 1) {
        debug$j("Setting block property");
        const {
          children,
          ...nextRest
        } = value;
        ...
        if (onSamePath) {
          debug$j("On same path, restoring previous selection");
          Transforms.select(editor, prevSel);
        }
      } else {
        debug$j("Setting non-text property");
        editor.apply({
          type: "set_node",
          path: targetPath,
          properties: {},
  ...
  function unsetPatch(editor, patch, previousPatch2) {
    ...
    if (patch.path.length === 1) {
      ...
      if (index === -1) {
        debug$j("Could not find block to unset at path ".concat(JSON.stringify(patch.path)));
        return false;
      }
      debug$j("Removing block at path [".concat(index, "]"));
      debugState(editor, "before");
    ...
    if (editor.isTextBlock(block) && patch.path[1] === "children" && patch.path.length === 3) {
      ...
      if (childIndex === -1) {
        debug$j("Could not find child to unset at path ".concat(JSON.stringify(targetPath)));
        return false;
      }
      debug$j("Unsetting child at path ".concat(JSON.stringify(targetPath)));
      debugState(editor, "before");
      if (prevSel && onSamePath && editor.isTextBlock(block)) {
        ...
        if (needToAdjust) {
          ...
          if (isMergeUnset) {
            ...
            debug$j("Adjusting selection for merging of nodes");
            prevSel.focus = {
              ...prevSel.focus
            };
  ...
  return function (editor, patch) {
    let changed = false;
    debug$j("\n\nNEW PATCH =============================================================");
    debug$j(JSON.stringify(patch, null, 2));
    try {
      switch (patch.type) {
        case "insert":
        ...
        case "diffMatchPatch":
          ...
          break;
        default:
          debug$j("Unhandled patch", patch.type);
      }
    } catch (err) {
      console.error(err);
    }
...
function debugState(editor, stateName) {
  debug$j("Children ".concat(stateName, ":"), JSON.stringify(editor.children, null, 2));
  debug$j("Selection ".concat(stateName, ": "), JSON.stringify(editor.selection, null, 2));
}
...
const debug$i = debugWithName("plugin:withUndoRedo");
  ...
  return editor => {
    if (patches$) {
      editor.subscriptions.push(() => {
        ...
        return () => {
          debug$i("Unsubscribing to patches");
          sub.unsubscribe();
        };
      });
    }
    ...
    editor.apply = op => {
      ...
      if (save) {
        ...
        if (step && merge) {
          ...
          undos.push(stp);
          debug$i("Created new undo step", step);
        }
            ...
            Editor.withoutNormalizing(editor, () => {
              transformedOperations.map(Operation.inverse).reverse().forEach(op => {
                try {
                  ...
                  debug$i("Could not perform undo step", err);
            ...
            Editor.withoutNormalizing(editor, () => {
              transformedOperations.forEach(op => {
                try {
                  ...
                  debug$i("Could not perform redo step", err);
...
function transformOperation(editor, patch, operation) {
  ...
  if (patch.type === "unset" && patch.path.length === 0) {
    debug$i("Adjusting selection for unset everything patch and ".concat(operation.type, " operation"));
    return [operation];
  }
  if (patch.type === "diffMatchPatch") {
    ...
    if (Element$1.isElement(block) && Array.isArray(block.children)) {
      ...
      if (!parsed) {
        debug$i("Could not parse diffMatchPatch", patch);
        return [operation];
      }
      const distance = parsed.length2 - parsed.length1;
...
function adjustBlockPath(editor, patch, operation, level) {
  ...
  if (myIndex >= 0 && operation.type !== "set_selection" && Array.isArray(operation.path) && operation.path[0] !== void 0 && operation.path[0] >= myIndex + level) {
    ...
    debug$i("Adjusting ".concat(operation.type, " for block ").concat(patch.type), operation.path, newPath);
...
const debug$h = debugWithName("plugin:withPatches");
function createWithPatches(_ref5) {
  ...
  return function withPatches(editor) {
      ...
      editor.subscriptions.push(() => {
        ...
        const sub = patches$.pipe(tap(_ref6 => {
          ...
          if (remotePatches.length !== 0) {
            debug$h("Remote patches", patches);
            Editor.withoutNormalizing(editor, () => {
              remotePatches.forEach(patch => {
                debug$h("Handling remote patch ".concat(JSON.stringify(patch)));
                  ...
                  withoutSaving(editor, () => {
                    withPreserveKeys(editor, () => {
                      try {
                        ...
                        debug$h("Got error trying to create operations from patch");
                        console.error(err);
                      }
                    });
                  });
    ...
    editor.apply = operation => {
      ...
      if (!isPatching(editor)) {
        debug$h("Editor is not producing patch for operation ".concat(operation.type), operation);
        return editor;
      }
...
const debug$g = debugWithName("plugin:withPlaceholderBlock");
  ...
  return function withPlaceholderBlock(editor) {
    ...
    editor.onChange = () => {
      ...
      if (editor.children.length === 0) {
        debug$g("Inserting placeholder block");
        Transforms.deselect(editor);
...
const debug$f = debugWithName("plugin:withPortableTextBlockStyle");
      ...
      for (const op of editor.operations) {
        if (op.type === "split_node" && op.path.length === 1 && editor.isTextBlock(op.properties) && op.properties.style !== defaultStyle && op.path[0] === path[0] && !Path.equals(path, op.path)) {
          ...
          if (Text.isText(child) && child.text === "") {
            debug$f("Normalizing split node to ".concat(defaultStyle, " style"), op);
            Transforms.setNodes(editor, {
              style: defaultStyle
            }, {
              at: [op.path[0] + 1],
    ...
    editor.pteToggleBlockStyle = blockStyle => {
      ...
      selectedBlocks.forEach(_ref9 => {
        ...
        if (editor.isTextBlock(node) && node.style === blockStyle) {
          debug$f("Unsetting block style '".concat(blockStyle, "'"));
          Transforms.setNodes(editor, {
            ...node,
            style: defaultStyle
          }, {
          ...
          if (blockStyle) {
            debug$f("Setting style '".concat(blockStyle, "'"));
          } else {
            debug$f("Setting default style", defaultStyle);
          }
          Transforms.setNodes(editor, {
            ...node,
...
const debug$e = debugWithName("plugin:withPortableTextLists");
...
function createWithPortableTextLists(types) {
  return function withPortableTextLists(editor) {
    editor.pteToggleListItem = listItemStyle => {
      ...
      if (isActive) {
        debug$e("Remove list item '".concat(listItemStyle, "'"));
        editor.pteUnsetListItem(listItemStyle);
      } else {
        debug$e("Add list item '".concat(listItemStyle, "'"));
    ...
    editor.pteUnsetListItem = listItemStyle => {
      ...
      selectedBlocks.forEach(_ref10 => {
        ...
        if (editor.isListBlock(node)) {
          ...
          debug$e("Unsetting list '".concat(listItemStyle, "'"));
    ...
    editor.pteSetListItem = listItemStyle => {
      ...
      selectedBlocks.forEach(_ref11 => {
        ...
        debug$e("Setting list '".concat(listItemStyle, "'"));
        Transforms.setNodes(editor, {
          ...node,
          level: 1,
    ...
    editor.pteIncrementBlockLevels = reverse => {
      ...
      selectedBlocks.forEach(_ref13 => {
        ...
        if (editor.isListBlock(node)) {
          ...
          if (reverse) {
            level--;
            debug$e("Decrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)));
          } else {
            level++;
            debug$e("Incrementing list level", Math.min(MAX_LIST_LEVEL, Math.max(1, level)));
          }
          Transforms.setNodes(editor, {
...
function createWithPortableTextMarkModel(types) {
  return function withPortableTextMarkModel(editor) {
    ...
    editor.normalizeNode = nodeEntry => {
      ...
      if (isSpan || isTextBlock) {
        if (!isTextBlock && !Array.isArray(node.marks)) {
          debug$d("Adding .marks to span node");
          Transforms.setNodes(editor, {
            marks: []
          }, {
            at: path
        ...
        for (const op of editor.operations) {
          if (op.type === "merge_node" && op.path.length === 1 && "markDefs" in op.properties && op.properties._type === types.block.name && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] - 1 >= 0) {
            ...
            debug$d("Copying markDefs over to merged block", op);
            if (editor.isTextBlock(targetBlock)) {
          ...
          if (op.type === "split_node" && op.path.length === 1 && Element$1.isElementProps(op.properties) && op.properties._type === types.block.name && "markDefs" in op.properties && Array.isArray(op.properties.markDefs) && op.properties.markDefs.length > 0 && op.path[0] + 1 < editor.children.length) {
            const [targetBlock, targetPath] = Editor.node(editor, [op.path[0] + 1]);
            debug$d("Copying markDefs over to split block", op);
            if (editor.isTextBlock(targetBlock)) {
              const oldDefs = Array.isArray(targetBlock.markDefs) && targetBlock.markDefs || [];
              Transforms.setNodes(editor, {
    ...
    editor.pteToggleMark = mark => {
      ...
      if (isActive) {
        debug$d("Remove mark '".concat(mark, "'"));
        Editor.removeMark(editor, mark);
      } else {
        debug$d("Add mark '".concat(mark, "'"));
      ...
      for (const [block, path] of blocks) {
        if (editor.isTextBlock(block)) {
          ...
          if (!isEqual(newMarkDefs, block.markDefs)) {
            debug$d("Removing markDef not in use");
            Transforms.setNodes(editor, {
              markDefs: newMarkDefs
            }, {
              at: path
...
const debug$c = debugWithName("plugin:withPortableTextSelections");
function createWithPortableTextSelections(change$, types) {
  return function withPortableTextSelections(editor) {
    const emitPortableTextSelection = () => {
      ...
      debug$c("Emitting selection ".concat(JSON.stringify(ptRange || null)));
      if (ptRange) {
        change$.next({
          type: "selection",
          selection: ptRange
  ...
  return function withSchemaTypes(editor) {
    ...
    editor.apply = op => {
      ...
      if (isInsertTextWithoutType) {
        ...
        op.node = newNode;
        debug$b("Setting span type to child without a type", op);
      }
      apply(op);
    };
    return editor;
...
const debug$a = debugWithName("plugin:withUtils");
    ...
    editor.pteExpandToWord = () => {
      ...
      if (selection && !Range.isExpanded(selection)) {
        ...
        if (!textNode || !Text.isText(textNode) || textNode.text.length === 0) {
          debug$a("pteExpandToWord: Can't expand to word here");
          return;
        }
        const {
          focus
        ...
        if (!(newStartOffset === newEndOffset || isNaN(newStartOffset) || isNaN(newEndOffset))) {
          debug$a("pteExpandToWord: Expanding to focused word");
...
const debug$9 = debugWithName("plugin:withHotKeys");
    ...
    editor.pteWithHotKeys = event => {
          ...
          for (const hotkey in activeHotkeys[cat]) {
            ...
            if (isHotkey(hotkey, event.nativeEvent)) {
              ...
              if (possibleMark) {
                ...
                debug$9("HotKey ".concat(hotkey, " to toggle ").concat(mark));
                editor.pteToggleMark(mark);
              }
            }
          }
      ...
      if (isBackspace && editor.selection && editor.selection.focus.path[0] > 0 && Range.isCollapsed(editor.selection)) {
        ...
        if (prevBlock && focusBlock && Editor.isVoid(editor, prevBlock) && editor.selection.focus.offset === 0) {
          debug$9("Preventing deleting void block above");
          event.preventDefault();
      ...
      if (isDelete && editor.selection && editor.selection.focus.offset === 0 && Range.isCollapsed(editor.selection) && editor.children[editor.selection.focus.path[0] + 1]) {
        ...
        if (nextBlock && focusBlock && !Editor.isVoid(editor, focusBlock) && Editor.isVoid(editor, nextBlock)) {
          debug$9("Preventing deleting void block below");
          event.preventDefault();
          event.stopPropagation();
          Transforms.removeNodes(editor, {
            match: n => n === focusBlock
...
const debug$8 = debugWithName("plugin:withInsertData");
function createWithInsertData(change$, schemaTypes, keyGenerator) {
  return function withInsertData(editor) {
    ...
    editor.insertPortableTextData = data => {
      ...
      debug$8("Inserting portable text from ".concat(origin, " event"), pText);
      if (pText) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          if (!validation.valid) {
            ...
            debug$8("Invalid insert result", validation);
            return false;
          }
    ...
    editor.insertTextOrHTMLData = data => {
      var _a;
      if (!editor.selection) {
        debug$8("No selection, not inserting");
        return false;
      }
      change$.next({
        type: "loading",
      ...
      if (html || text) {
        ...
        debug$8("Inserting ".concat(insertedType, " fragment at ").concat(JSON.stringify(editor.selection)));
...
function mixMarkDefs(editor, fragment) {
  ...
  if (editor.isTextBlock(focusBlock) && editor.isTextBlock(fragment[0])) {
    ...
    debug$8("Mixing markDefs of focusBlock and fragments[0] block", markDefs, fragment[0].markDefs);
    Transforms.setNodes(editor, {
...
const debug$7 = debugWithName("component:PortableTextEditor:SlateContainer");
function SlateContainer(props) {
  ...
  useEffect(() => {
    debug$7("Re-initializing plugin chain");
  ...
  const initialValue = useMemo(() => value ? toSlateValue(value, {
    schemaTypes: portableTextEditor.schemaTypes
  }, KEY_TO_SLATE_ELEMENT) : [slateEditor.createPlaceholderBlock()],
  ...
  useEffect(() => {
    return () => {
      debug$7("Destroying Slate editor");
      slateEditor.destroy();
    };
  }, [slateEditor]);
...
const debug$6 = debugWithName("hook:useSyncValue");
function useSyncValue(props) {
  ...
  return useMemo(() => (value, userCallbackFn) => {
    const callbackFn = () => {
    ...
    if (isPending.current && !readOnly) {
      debug$6("Has local patches");
      return;
    }
    if (previousValue.current === value) {
      debug$6("Value is the same object");
      return;
    }
    previousValue.current = value;
    if (!value || value.length === 0) {
      debug$6("Value is empty");
        ...
        withoutPatching(slateEditor, () => {
          Editor.withoutNormalizing(slateEditor, () => {
            ...
            if (slateValueFromProps.length < childrenLength) {
              Array.from(Array(childrenLength - slateValueFromProps.length)).forEach((_, index) => {
            ...
            slateValueFromProps.forEach((c, i) => {
              ...
              if (oldBlock && !isEqual(c, oldBlock)) {
                debug$6("Replacing changed block", oldBlock);
                ...
                withPreserveKeys(slateEditor, () => {
                  Transforms.insertNodes(slateEditor, c, {
                    at: [i]
                  });
                });
                if (currentSel) {
...
const debug$5 = debugWithName("component:PortableTextEditor:Synchronizer");
function Synchronizer(props) {
  ...
  useEffect(() => {
    startTransition(() => {
      debug$5("Value from props changed, syncing new value");
      syncValue(value);
    });
    change$.next({
      type: "value",
  ...
  const onFlushPendingPatches = useCallback(() => {
    ...
    debug$5("Flushing pending patches", finalPatches);
  ...
  useEffect(() => {
    debug$5("Subscribing to editor changes$");
    ...
    return () => {
      debug$5("Unsubscribing to changes$");
      sub.unsubscribe();
    };
...
const debug$4 = debugWithName("component:PortableTextEditor:Validator");
function Validator(props) {
  ...
  const previousValue = useRef();
  const [invalidValueResolution, setInvalidValueResolution] = useState(null);
  useEffect(() => {
    const hasNewBlock = value && previousValue.current && value.length > previousValue.current.length;
  ...
  if (invalidValueResolution) {
    return /* @__PURE__ */jsx(Fragment, {
      children: invalidValueResolution.description
    });
  }
  return /* @__PURE__ */jsx(Fragment, {
    children: props.children
...
class PortableTextEditor extends React.Component {
  ...
  render() {
    ...
    return /* @__PURE__ */jsx(Validator, {
      portableTextEditor: this,
      keyGenerator,
      value,
      children: /* @__PURE__ */jsx(SlateContainer, {
        keyGenerator,
        maxBlocks,
        patches$: _patches$,
        portableTextEditor: this,
